    ...
    tasks: {
      byId: {
        [taskId]: {
          ...
          progress: 100,
          result: {
            items: [{ url: 'https://example.com/e2e-project-task-result-1.png' }],
            coverUrl: 'https://example.com/e2e-project-task-result-1.png',
            outputType: 'main',
            meta: {}
          },
        ...
        [secondTaskId]: {
          ...
          progress: 100,
          result: {
            items: [{ url: 'https://example.com/e2e-project-task-result-2.png' }],
            coverUrl: 'https://example.com/e2e-project-task-result-2.png',
            outputType: 'main',
            meta: {}
          },
        ...
        [thirdTaskId]: {
          ...
          progress: 100,
          result: {
            items: [{ url: 'https://example.com/e2e-project-task-result-3.png' }],
            coverUrl: 'https://example.com/e2e-project-task-result-3.png',
            outputType: 'main',
            meta: {}
          },
          deliveryStatus: 'pending_review',
...
describe('project-detail.vue project-level batch review action', () => {
  const taskId = 'task-e2e-project-approve-001'
  const batchId = 'batch-e2e-project-approve-001'
  const projectId = 'proj-e2e-project-approve-001'
  let page = null
  ...
  it('approves deliverable batches in project and refreshes summary/history', async () => {
    const approveButton = await page.$('#e2e-approve-deliverable-batches-in-project-btn')
    ...
    expect(summaryText).toContain('Last Action: Approve Deliverable Batches In Project')
    expect(summaryText).toContain('Action Type: approve_deliverable_batches_in_project')
    ...
    expect(recentText).toContain('approve_deliverable_batches_in_project')

    const latestState = await program.callUniMethod('getStorageSync', 'main_chain_state')
    const firstTask = latestState && latestState.tasks && latestState.tasks.byId && latestState.tasks.byId[taskId]
    expect(firstTask).toBeTruthy()
    expect(firstTask.deliveryStatus).toBe('approved')
    expect(firstTask.deliveryConfirmedAt).toBeTruthy()
  })
})
