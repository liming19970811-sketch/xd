...
const taskId = process.env.RESULT_TASK_ID || 'task-pw-result-needs-revision-001'
      ...
      byId: {
        [currentTaskId]: {
          ...
          result: {
            items: [
              {
                fileUrl: 'https://example.com/result-needs-revision.png'
              }
            ],
            coverUrl: 'https://example.com/result-needs-revision.png',
            outputType: 'main',
            meta: {}
          },
          error: {
...
async function run() {
  ...
  console.log(`[pw-result-needs-revision-smoke] node=${process.version}`)
  console.log(`[pw-result-needs-revision-smoke] url=${resultUrl.toString()}`)
  console.log(`[pw-result-needs-revision-smoke] headless=${headless}`)
  ...
  try {
    ...
    console.log(`[pw-result-needs-revision-smoke] status=${status}`)
    ...
    await page.waitForLoadState('networkidle', { timeout: 15000 }).catch((error) => {
      console.log(`[pw-result-needs-revision-smoke] networkidle skipped: ${error.message}`)
    })

    await page.waitForURL(/#\/pages\/result\/result\?taskId=/, { timeout: 10000 })

    const markButton = page.locator('#e2e-mark-needs-revision-btn')
    await markButton.waitFor({ state: 'visible', timeout: 10000 })

    const markText = await markButton.innerText()
    console.log(`[pw-result-needs-revision-smoke] markText=${markText}`)
    assert(markText.includes('标记待修改'), 'Expected mark needs revision button to be visible')
    ...
    console.log(`[pw-result-needs-revision-smoke] deliveryLine=${deliveryText}`)
    assert(
      deliveryText.includes('交付审核状态'),
      'Expected delivery review status line to be visible'
    )

    console.log('[pw-result-needs-revision-smoke] PASS')
  } finally {
    await browser.close()
  }
}
...
run().catch((error) => {
  console.error('[pw-result-needs-revision-smoke] FAIL')
  console.error(error)
  process.exit(1)
})
