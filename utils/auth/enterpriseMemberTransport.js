import { getCurrentSession } from './authSessionService.js'
...
function fail(errorCode = 'cloud_call_failed', message = 'enterprise_member cloud function call failed') {
...
function normalizeCloudError(error = {}) {
  ...
  const message = String(error.message || error.errMsg || error.msg || code || 'enterprise_member cloud function call failed')
...
function normalizeResponse(response = {}) {
  if (!response || typeof response !== 'object' || !('result' in response)) {
    return fail('cloud_response_invalid', 'enterprise_member cloud function response is invalid')
  }
  const result = response.result
  return result && typeof result === 'object'
    ? result
    : fail('cloud_response_invalid', 'enterprise_member cloud function result is invalid')
}

export async function callEnterpriseMember(action = '', data = {}) {
  ...
  if (isH5Platform()) {
    ...
    try {
      return normalizeResponse(await callCloudWebFunction('enterprise_member', payload))
    } catch (error) {
      return normalizeCloudError(error)
    }
  }

  if (!canUseWxCloud()) return fail('cloud_unavailable', '当前环境不可调用 enterprise_member 云函数')
  try {
