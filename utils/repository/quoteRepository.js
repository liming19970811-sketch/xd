...
export function normalize(record = {}, options = {}) {
  return normalizeSaasRecord({ ...record, quoteId: record.quoteId || '' }, options)
}

export function getList() {
  const context = getCurrentEnterpriseContext()
  return filterTenantData(readStore().quotes, getCurrentEnterpriseId()).map((item) => normalize(item, context))
}
...
export function create(input = {}) {
  ...
  const record = normalize({ ...protectedInput, quoteId: input.quoteId || `quote_${Date.now()}`, status: input.status || 'draft', createdAt: input.createdAt || now, updatedAt: input.updatedAt || now }, context)
  writeLocalStorage(STORAGE_KEY, { ...store, quotes: [record, ...store.quotes.filter((item) => item.quoteId !== record.quoteId || resolveTenantId(item) !== context.enterpriseId)], schemaVersion: ENTERPRISE_SCHEMA_VERSION })
  return record
}
...
export function update(quoteId = '', patch = {}) {
  ...
  const current = records.find((item) => item.quoteId === quoteId)
  if (!current) return null
  const context = getCurrentEnterpriseContext()
  const protectedRecord = protectTenantUpdate(current, patch, context.enterpriseId)
  if (!protectedRecord) return null
  const record = normalize({ ...protectedRecord, quoteId }, { ...context, updatedBy: patch.updatedBy || patch.userId, touchUpdatedAt: new Date().toISOString() })
  writeLocalStorage(STORAGE_KEY, { ...store, quotes: store.quotes.map((item) => item.quoteId === quoteId && resolveTenantId(item) === context.enterpriseId ? record : item), schemaVersion: ENTERPRISE_SCHEMA_VERSION })
  return record
}
